...
def load_local_snow_depth_history(path=LOCAL_HISTORY_JSON_PATH):
    ...
    if not os.path.exists(path):
        raise FileNotFoundError(
            f"mmnv1.json not found at {path!r}. Expected it checked out "
            "alongside this script at the repo root - check the checkout "
            "step actually pulled it (e.g. sparse-checkout paths, working "
    ...
    if file_size == 0:
        raise ValueError(
            f"mmnv1.json at {path!r} is 0 bytes. The file exists but has "
            "no content - this happens if something upstream (checkout, "
            "cache restore, an artifact download, etc.) replaced it with "
            "an empty placeholder before this script ran."
        )
    ...
    try:
        ...
        raise ValueError(
            f"mmnv1.json at {path!r} ({file_size} bytes) is not valid "
            f"JSON: {error}. First 200 chars of the file: {preview!r}"
        ) from error
